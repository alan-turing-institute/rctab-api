...
class TestUsageRouteLocks:
    ...
    async def test_post_usage_uses_advisory_lock(
        self, mock_advisory_lock: AsyncMock, test_db: AsyncConnection
    ) -> None:
        ...
        with (
            patch("rctab.routers.accounting.usage.UsageEmailContextManager"),
            patch("rctab.routers.accounting.usage.delete_usage"),
            patch("rctab.routers.accounting.usage.insert_subscriptions_if_not_exists"),
            patch("rctab.routers.accounting.usage.insert_usage"),
            patch("rctab.routers.accounting.usage.refresh_desired_states"),
        ):

            # Execute
            await post_usage(usage_data, {"mock": "auth"}, conn=test_db)
    ...
    async def test_post_monthly_usage_uses_advisory_lock(
        ...
        with (
            ...
            # Execute
            await post_monthly_usage(usage_data, {"mock": "auth"}, conn=test_db)
...
class TestCostRecoveryLocks:
    ...
    async def test_cost_recovery_app_uses_lock(
        self, mock_calc_cost_recovery: AsyncMock, mock_advisory_lock: AsyncMock
    ) -> None:
        ...
        # Execute
        await calc_cost_recovery_app(
            recovery_period, {"mock": "auth"}, conn=AsyncMock()
        )
    ...
    async def test_cost_recovery_cli_uses_lock(
        ...
        # Execute
        await post_calc_cost_recovery_cli(recovery_period, mock_user, conn=AsyncMock())
...
class TestMaterializedViewLocks:
    ...
    async def test_materialized_view_refresh_uses_lock(
        ...
        with patch.object(test_db, "execute", new_callable=AsyncMock) as mock_execute:
            ...
            sql_call = str(mock_execute.call_args[0][0])
...
class TestLockErrorHandling:
    ...
    async def test_usage_route_handles_lock_timeout(
        ...
        mock_advisory_lock.side_effect = HTTPException(
            status_code=503,
            detail=(
                "Could not acquire lock for operation "
                "'usage_upload_2021-01-01_2021-01-01'. "
                "Another process may be performing the same operation."
            ),
        )
    ...
    @pytest.mark.asyncio
    @patch("rctab.routers.accounting.cost_recovery.advisory_lock_nowait")
    async def test_cost_recovery_handles_concurrent_execution(
        self, mock_advisory_lock: AsyncMock
    ) -> None:
        ...
        mock_advisory_lock.side_effect = HTTPException(
            status_code=409,
            detail=(
                "Operation 'cost_recovery_2021_05' is already in progress. "
